...
\item[(c)] Suppose there is an assignment $\P^\prime \neq \P$ to $\X$ that satisfies both (a) and (b). As we deduce from the part (c) of Lemma \ref{lemmaone}, $\P$ is the only assignment to $\X$ such that $\A\not\models\exists\x\:\psi_q(\P)$, so necessarily $\A\models\exists\x\:\psi_q(\P^\prime)$. Since $\P^\prime$ assigned to $\X$ satisfies (a), then $\A\models\exists\x\:\varphi(\x,\P^\prime).$ Let $q^\prime$ be such that $\A\models\exists\x\:\varphi_{q^\prime}(\x,\P^\prime).$ By the choice of $q,$ then $q^\prime \geq q.$
\end{enumerate}
