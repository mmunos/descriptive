...
\marcelo{Esto quedo listo con el ejemplo que puso Martin y la pasada que hice sobre el}
...
\item[$\checkmark$] p.32/33, example 6.2: I don't see why the formula given for dag's does not work for arbitrary
...
\marcelo{Esta bien la respuesta en la carta. La agregue al ejemplo para que quedara mas claro}
\item[$\checkmark$] p.33: ``executes exactly $n^k$ steps for a fixed $k \geq 1$'' $\rightarrow$ ``executes exactly $n^k$
steps on large
...
\begin{itemize}
	...
	\marcelo{corregi estos errores en el paper}
...
\cristian{No tiene mucho sentido estos comentarios del reviewer ya que va más alla del paper. Para dejarlo tranquilo, agregue un último comentario en las conclusiones diciendo que sería interesante estudiar estos problemas en el contexto de enumeración. También agregue la respuesta a la carta.}

\item[$\checkmark$] For the most involved proofs, you could describe informally your proof strategy in a few lines to help the reader. You may also add some example to illustrate the different fragment of QSO (you already do it sometimes and it is very helpful).
...
\cristian{El paper ya tiene demasiados ejemplos y ya esta demasiado largo. En la carta le respondí que lamentablemente, debido a que el paper esta demasiado largo, decidimos no agregar mas ejemplos.}
...
\cristian{Le respondí diciendo que es mencionado en la respuesta al otro reviewer.}
\item[$\checkmark$] page 28:
...
\cristian{Agregue un parrafo al final de la sección 5 mencionando esto. De hecho, desde la demostración uno puede demostrar que las dos classes son iguales (la suma de afuera no te agrega expresividad). Le respondí esto mismo en la carta.}
...
\martin{conversemos esto}
\cristian{Tiene razón y eliminé esta oración al final del paper.}
\end{itemize}
