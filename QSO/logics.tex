...
We introduce the operator $[\textbf{alfp}_{(x_1,\ldots,x_k),R}\varphi(x_1,\ldots,x_k,R)\mid\alpha(u_1,\ldots,u_k,R,p)](y_1,\ldots,y_k)$ which uses a FO-formula $\varphi$ as well as a QSO-formula $\alpha$ and takes as input a placeholder variable $p$, which is interpreted as a constant. The function $\zeta_{\varphi,\alpha}:2^{A^k} \to \nat$ is defined as $\zeta_{\varphi,\alpha}(T_{\varphi}(X)) = \sem{\alpha'}(\A,R)$ where $\alpha'$ is obtained by replacing every instance of $p$ by $\zeta_{\varphi,\alpha}(X)$. 
