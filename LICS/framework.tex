...
Suppose now that we want to count the number of cliques in $\fG$ (i.e. set of nodes where every pair of nodes is connected). We can easily compute this function with the following formula:
$$
...
The permanent is a relevant function over matrices that has found many applications in different areas~\cite{permanent-applications} and it was one of the first function that was shown to be difficult for counting~\cite{Valiant79} (i.e. $\shp$-complete). 
For a binary relation symbol $S$, let $\op{permut}(S)$ be the $\fo$-formula that is true iff $S$ is a permutation, i.e. bijective function, (the definition of $\op{permut}(S)$ is straightforward and we left this exercise to the reader).
