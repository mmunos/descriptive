...
%(i.e. set of nodes where every pair of nodes is connected). 
We can 
%easily 
...
%~\cite{paper-that-shows-that-this-problem-is-sharpP-hard}.
\end{example}
...
We consider several fragments [and extensions] of $\qso$, which are obtained by restricting the syntax of the Boolean formulae or the use of the quantitative quantifiers.
...
We also consider fragments [and extensions] of $\qso$ by further restricting the Boolean part of the logic.
If $\LL$ is a fragment [or extension] of $\so$, then we define the quantitative logic $\qso(\LL)$ to be the fragment of $\qso$ obtained by restricting $\varphi$ in \eqref{syntax} to be a formula in $\LL$. Moreover, we also [consider] other fragments [or extension] of $\qso$ by using the same idea. 
%In general, if $\FF$ is a fragment of $\qso$ we define $\FF(\LL)$ to be the fragment of $\FF$ obtained by restricting Boolean formulae to $\LL$.
...
%, $\eqso(\fo)$ of $\qfo$ and $\eqso$ where $\varphi$ is restricted to $\fo$ formulae. 

%\marcelo{IMPORTANTE: Tenemos que decir que $\qfo(\fo)$ permite variables libre de segundo orden en las formulae de primer orden. Hay que aclarar esto tambien para PFP y otras logicas.}

In the following section, we use different fragments [and extensions] of $\qso$ to capture counting complexity classes. 
%variate $\FF$ and $\LL$ to capture several function complexity classes. 
But before doing this, we 
%devote the next subsection to 
...
%and then counting how many extensions of an structure satisfy the formula (i.e. by instantiating the free variables). 
Formally, Saluja et. al \cite{SalujaST95} define a family of counting classes $\#\LL$ for a fragment $\LL$ of $\fo$. [This definition can also be expanded to include extensions of $\fo$ as it was done in \cite{ComptonG96}.] For a formula $\varphi(\bar{x},\bar{X})$ over $\R$, the function $f_{\varphi(\bar x, \bar X)}$ is defined as
$
...
%\martin{La notacion $f_{\psi(x,X)}$ la inventamos}
