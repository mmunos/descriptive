...
\end{multline}
where $\varphi$ is an $\so$-formula over $\R$, $s \in \bbN$, $x \in \fv$ and $X \in \sv$. Moreover, if $\R$ is not mentioned, then $\qso$ refers to the union of the sets of $\qso$ formulae over all posible relational signatures.
...
Suppose now that we want to compute the permanent of an $n$-by-$n$ 0-1 matrix $A$, which is defined as:
\begin{eqnarray*}
...
In this subsection, we discuss some previous frameworks proposed in the literature and how they differ from our approach.
