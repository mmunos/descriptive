...
\noindent {\em Part (1).}  Let $\alpha =  \sa{\bar{X}}\sa{\bar{x}}\exists\bar{y}\, \varphi(\bar{X},\bar{x},\bar{y})$ where $\varphi$ is quantifier-free and with  atomic formulas in $\fo$. We call this syntax {\em extended quantifier-free} from now on. 
Note that, if $\alpha$ is of the form $\alpha = \sa{\bar{x}}\exists\bar{y}\,\varphi(\bar{x},\bar{y})$ (i.e. $\length(\bar{X}) = 0$), we can define $\kappa(\alpha) = \sa{\bar{x}}\exists\bar{y}[\varphi(\bar{x},\bar{y})\wedge\exists\bar{z}(\varphi(\bar{z},\bar{y})\wedge\bar{z} < \bar{x})]$, which is in $\eqso(\logex{1})$ and fulfills the desired condition. 
...
\begin{itemize}
	...
	Thus, the formula $\varphi(\bar{X},\bar{x},\bar{y})$ is then redefined as:
	$$
	...
	Then, by redefining the conjuncts $\varphi_i(\bar{X},\bar{x},\bar{y})$ of $\varphi$ to $\theta^j(\bar{x},\bar{y})\wedge \varphi_i(\bar{X},\bar{x},\bar{y})$, we can suppose that each $\varphi_i^{\fo}(\bar{x},\bar{y})$ defines an ordered partition over the variables in $(\bar{x},\bar{y})$.
...
\end{itemize}
It is important to notice that the resulting $\varphi$ is equivalent to the initial one, and it is still an extended quantifier-free formula. From now on, we assume that each $\varphi_i(\bar{X},\bar{x},\bar{y})$ satisfies conditions (a), (b) and (c). Furthermore, by condition (a) we have that:
$$
\varphi_i(\bar{X},\bar{x},\bar{y}) \; = \; \varphi^{\fo}_i(\bar{x},\bar{y}) \wedge \varphi^{+}_i(\bar{X}, \bar{y}) \wedge \varphi^{-}_i(\bar{X},\bar{y}) 
$$
where $\varphi^{+}$ and $\varphi^{-}_i$ do not depend on the variables $\bar{x}$.
...
The previous claim and proof motivates the following definitions, in particular, a generalization of the sequence $\bar{B}$. 
...
Moreover, let $\bar{v}$ be the sequence of all different first-order variable mentioned in $\varphi_i^{+}$ and let $\bar{u}$ be a sequence of variables disjoint from $\bar{v}$ such that $(\bar{v},\bar{u})$ and $(\bar{x},\bar{y})$ have the same set of variables. Then define $v^*$ to be the first-order assignment for $\A$ for which we have that $v^{*}(\bar{v})$ is the lexicographically smallest tuple that satisfies $(\A,v^{*})\models\exists\bar{u} \varphi^{\fo}_i(\bar{u},\bar{v})$.
Indeed, one ca define a formula $\mu_i$ that defines $v^*$ as follows:
$$
...
$$ 
In other words, we have that $(\A,v)\models \mu_i(\bar{v})$ if, and only if, $v(\bar{v}) = v^{*}(\bar{v})$ for every assignment $v$. 
Finally, define $\bar{B}^* = \bar{B}^{v^{*}}$ and $V^*$ as some second-order assignment for $\A$ for which $V^*(\bar{X}) = \bar{B}^*$.

Now, we have all the tools to introduce the main formula of the proof. 
Intuitively, the pair $(V^*, v^*)$ is the minimum assignment that satisfies $\varphi_i(\bar{X}, \bar{x})$ and our candidate to be removed from the semantics of $\varphi(\bar{X}, \bar{x})$. Towards this goal, we can define a formula $\psi_i(\bar{X},\bar{x})$ that has the following key property: 
\begin{center}
if $\varphi_i(\bar{X}, \bar{x}, \bar{y})$ is satisfiable, then $(\A,V,v) \models\varphi_i(\bar{X},\bar{x},\bar{y})$ if, and only if, $V(\bar{X}) \neq V^*(\bar{X})$ or $v(\bar{x}) \neq v^*(\bar{x})$ for every $(V,v)$. 
\end{center}
That is, its exclude somehow the pair $(V^*, v^*)$. This property can be defined in logic as follows:
\begin{align}
\psi_i(\bar{X},\bar{x}) = & \;\; \big( \, \exists\bar{x}\exists\bar{y}\, \varphi^{\fo}_i(\bar{x},\bar{y}) \,\big) \rightarrow  \label{eq:part1} \\
& \hspace{1cm} \Big( \; 
\exists\bar{x}'(\exists\bar{y}'\varphi_i(\bar{X},\bar{x}',\bar{y}')\wedge \bar{x}'<\bar{x}) \;\;  \vee   \label{eq:part2} \\
& \hspace{1.5cm} \exists\bar{v} (\mu_i(\bar{v})\wedge \big( \varphi^{+}_i(\bar{X},\bar{v}) \rightarrow \bigvee_{X \in \bar{X}} \exists\bar{z}\, ( \, X(\bar{z}) \wedge \bigwedge\limits_{X(\bar{w}) \, \in \, \varphi^{+}_i(\bar{X},\bar{v})}\bar{w}\neq\bar{z} \, ) \big) \Big)  \label{eq:part3}
\end{align}
...
Then, the conclusion of the implication at (\ref{eq:part2}) and (\ref{eq:part3}), take care that the $\varphi_i$ is only satisfied by a pair $(V, v)$ different from $(V^*, v^*)$.
Here, the first disjunct at (\ref{eq:part2}) check whether $(\A,V,v) \models\varphi_i(\bar{X},\bar{x},\bar{y})$ but $v(\bar{x})$ is not the lexicographically smallest tuple that have this property (i.e. $v(\bar{x}) \neq v^*(\bar{x})$).
The second disjunct at (\ref{eq:part3}) is more involved and it can be explained as follows:
