...
Now, for the formula $\sa{\bar{x}}\exists\bar{y}\,\varphi(\bar{x},\bar{y})$ note that if $\length(\bar{x}) = 0$, then the formula takes value at most 1, and if $\length(\bar{x}) = 1$, then the formula takes value at most $n$. Then we have that $\length(bar{x}) \geq 2$. Let $\psi = \bigvee_{i = 1}^{m}$ be an equivalent formula to $\varphi$ that is in DNF
...
\subsection*{Proof of Proposition \ref{prop:uhorn-pe}} %V.11
