...
We construct the equivalent formula as follows. We use a new function symbol $f$ with arity $\length{\bar{x}_1} + \length{\bar{x}_2}$. Let $\alpha_i'$ be obtained by performing the following replacements in $\alpha_i$, for each $i = 1,\ldots,n$.
\begin{align*}
...
\end{align*}
and lastly we produce the formula
\begin{align*}
\alpha'(\bar{x}_1) = \,&\sa{\bar{x}_2}\clfp{f(\bar{x}_1,\bar{x}_2): \beta(\bar{x}_1,\bar{x}_2,f,f_3,\ldots,f_n),\\
&\alpha_3(\bar{x}_3,f,f_3,\ldots,f_n),\ldots,\alpha_n(\bar{x}_n,f,f_3,\ldots,f_n)}\mult(\first(\bar{x}_1)\mult\last(\bar{x}_2)  \add (\neg\first(\bar{x}_1))\mult\first(\bar{x}_2))
\end{align*}
It can be seen that for all values of $f_1$, besides the first one one, are stored in the first assignment of $\bar{x}_2$, and the first itself is stored in the last assignment of $\bar{x}_2$. Moreover, all values of $f_2$, besides the first one, are stored in the last assignment of $\bar{x}_1$, and the first one itself is stored in the first assignment of $\bar{x}_1$. This schema is depicted in Table \ref{table-clfp}. Since $\alpha'(\bar{x}_1)$ is equivalent to $\alpha(\bar{x}_1)$, this concludes the proof.
\begin{table}[htbp]
	\centering
	\caption{Values of $f(\bar{x}_1,\bar{x}_2)$}
	\label{table-clfp}
	\begin{tabular}{crlllll}
		\multicolumn{1}{l}{} & \multicolumn{6}{c}{$\bar{x}_2$} \\ \cline{2-7} 
		\multicolumn{1}{c|}{\multirow{5}{*}{$\bar{x}_1$}} & \multicolumn{1}{c|}{$f_2$} &  &  &  & \multicolumn{1}{l|}{} & \multicolumn{1}{c|}{$f_1$} \\ \cline{2-2} \cline{7-7} 
		\multicolumn{1}{c|}{} & \multicolumn{1}{r|}{\multirow{4}{*}{$f_1$}} &  &  &  &  & \multicolumn{1}{l|}{} \\
		\multicolumn{1}{c|}{} & \multicolumn{1}{r|}{} &  &  &  &  & \multicolumn{1}{l|}{} \\
		\multicolumn{1}{c|}{} & \multicolumn{1}{r|}{} &  &  &  &  & \multicolumn{1}{l|}{} \\ \cline{3-7} 
		\multicolumn{1}{c|}{} & \multicolumn{1}{r|}{} & \multicolumn{5}{c|}{$f_2$} \\ \cline{2-7} 
	\end{tabular}
\end{table}
