...
We generalize $\clfp$ a bit. The formula is
\begin{align*}
\sa{t}\clfp{out(t): \,&\alpha_{T_0}(t,p,T_0,T_1,T_B,T_{\vdash},h,\hat{h},s_{q_1},\ldots,s_{q_{\ell}},out)\\
&\alpha_{T_1}(t,p,T_0,T_1,T_B,T_{\vdash},h,\hat{h},s_{q_1},\ldots,s_{q_{\ell}},out)\\
&\alpha_{T_B}(t,p,T_0,T_1,T_B,T_{\vdash},h,\hat{h},s_{q_1},\ldots,s_{q_{\ell}},out)\\
&\alpha_{T_{\vdash}}(t,p,T_0,T_1,T_B,T_{\vdash},h,\hat{h},s_{q_1},\ldots,s_{q_{\ell}},out)\\
&\alpha_{h}(t,p,T_0,T_1,T_B,T_{\vdash},h,\hat{h},s_{q_1},\ldots,s_{q_{\ell}},out)\\
&\alpha_{\hat{h}}(t,p,T_0,T_1,T_B,T_{\vdash},h,\hat{h},s_{q_1},\ldots,s_{q_{\ell}},out)\\
&\alpha_{s_{q_1}}(t,T_0,T_1,T_B,T_{\vdash},h,\hat{h},s_{q_1},\ldots,s_{q_{\ell}},out)\\
&\vdots \\
&\alpha_{s_{q_{\ell}}}(t,T_0,T_1,T_B,T_{\vdash},h,\hat{h},s_{q_1},\ldots,s_{q_{\ell}},out)\\
&\alpha_{out}(t,T_0,T_1,T_B,T_{\vdash},h,\hat{h},s_{q_1},\ldots,s_{q_{\ell}},out)}\mult last(t).
\end{align*}
We define:
\begin{align*}
\alpha_{T_0}(t,p,...) = \,&first(t) \wedge \neg E(p_1,p_2) + \\
&\sa{t'}(succ(t',t) \mult \hat{h}(t',p) \mult T_0(t',p)) + \\
&\bigplus_{\delta(q,a) = (q',0,op,v)}\sa{t'}(succ(t',t) \mult h(t',p) \mult T_a(t',p) \mult s_{q}(t')).
\end{align*}
We define:
\begin{align*}
\alpha_{T_1}(t,p,...) = \,&first(t) \wedge E(p_1,p_2) + \\
&\sa{t'}(succ(t',t) \mult \hat{h}(t',p) \mult T_1(t',p)) + \\
&\bigplus_{\delta(q,a) = (q',1,op,v)}\sa{t'}(succ(t',t) \mult h(t',p) \mult T_a(t',p) \mult s_{q}(t')).
\end{align*}
We define:
\begin{align*}
\alpha_{T_B}(t,p,...) = \,&first(t) \wedge (p \text{ is larger than input}) + \\
&\sa{t'}(succ(t',t) \mult \hat{h}(t',p) \mult T_B(t',p)) + \\
&\bigplus_{\delta(q,a) = (q',B,op,v)}\sa{t'}(succ(t',t) \mult h(t',p) \mult T_a(t',p) \mult s_{q}(t')).
\end{align*}
We define:
\begin{align*}
\alpha_{T_{\vdash}}(t,p,...) = \,&first(t) \wedge first(p) + \\
&\sa{t'}(succ(t',t) \mult \hat{h}(t',p) \mult T_{\vdash}(t',p)) + \\
&\bigplus_{\delta(q,a) = (q',\vdash,op,v)}\sa{t'}(succ(t',t) \mult h(t',p) \mult T_a(t',p) \mult s_{q}(t')).
\end{align*}
We define:
\begin{align*}
\alpha_{h}(t,p,...) = \,& first(t) \wedge second(p) + \\
&\bigplus_{\delta(q,a) = (q',b,\leftarrow,v)}\sa{t'}\sa{p'}(succ(t',t) \mult succ(p,p')\mult T_a(t',p') \mult h(t',p') \mult s_{q}(t')) + \\
&\bigplus_{\delta(q,a) = (q',b,\rightarrow,v)}\sa{t'}\sa{p'}(succ(t',t) \mult succ(p',p)\mult T_a(t',p') \mult h(t',p') \mult s_{q}(t')).
\end{align*}
We define:
\begin{align*}
\alpha_{\hat{h}}(t,p,...) = \,& first(t) \wedge (p\text{ is either first or greater than second}) + \\
&\bigplus_{\delta(q,a) = (q',b,\leftarrow,v)}\sa{t'}\sa{p'}\sa{p''}(succ(t',t) \mult T_a(t',p')  \mult h(t',p') \mult s_{q}(t') \mult succ(p'',p') \mult (p'' < p \vee p < p'')) + \\
&\bigplus_{\delta(q,a) = (q',b,\leftarrow,v)}\sa{t'}\sa{p'}\sa{p''}(succ(t',t) \mult T_a(t',p')  \mult h(t',p') \mult s_{q}(t') \mult succ(p',p'') \mult (p'' < p \vee p < p'')).
\end{align*}
We define:
\begin{align*}
\alpha_{q_0}(t,...) = \,& first(t) + \\
&\bigplus_{\delta(q,a) = (q_0,b,op,v)}\sa{t'}\sa{p'}(succ(t',t) \mult T_a(t',p') \mult h(t',p') \mult s_{q}(t')).
\end{align*}
For the remaining $q_i$ we define:
\begin{align*}
\alpha_{q_i}(t,...) = &\bigplus_{\delta(q,a) = (q_i,b,op,v)}\sa{t'}\sa{p'}(succ(t',t) \mult T_a(t',p') \mult h(t',p') \mult s_{q}(t')).
\end{align*}
And lastly we define:
\begin{align*}
\alpha_{out}(t,...) = &\bigplus_{\delta(q,a) = (q',b,op,0)}\sa{t'}\sa{p'}(succ(t',t) \mult h(t',p') \mult T_a(t',p') \mult 2\mult out(t')) + \\
&\bigplus_{\delta(q,a) = (q',b,op,1)}\sa{t'}\sa{p'}(succ(t',t) \mult h(t',p') \mult T_a(t',p') \mult (2\mult out(t')+1)) + \\ 
&\bigplus_{\delta(q,a) = (q',b,op,B)}\sa{t'}\sa{p'}(succ(t',t) \mult h(t',p') \mult T_a(t',p') \mult out(t')).
\end{align*}
