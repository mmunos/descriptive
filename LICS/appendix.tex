...
$$
\begin{claim}
	Let $\varphi_i(\bar{X},\bar{x},\bar{y}) = \varphi^{\fo}_i(\bar{x},\bar{y}) \wedge \varphi^{-}_i(\bar{X},\bar{x},\bar{y}) \wedge \varphi^{+}_i(\bar{X},\bar{x},\bar{y})$ be a quantifier-free formula that respects all the mentioned assumptions. For a given ordered structure $\A$ with domain $A$, if $\A\models\exists\bar{x}\exists\bar{y}\,\varphi^{\fo}_i(\bar{x},\bar{y})$, then there exists some $\bar{B}$ over $\A$ and $\bar{b}\in A^{\length(\bar{x})}$ such that $\A\models\varphi_i(\bar{B},\bar{b},\bar{a})$ for some $\bar{a}\in A^{\length(\bar{y})}$.
\end{claim}
\begin{proof}
...
\bigvee_{j = 1}^{j = i-1}\chi_j\vee\psi_i(\bar{X},\bar{x}))]].
$$
And lastly $\kappa(\alpha)$ is defined as $\kappa(\alpha) = \kappa(\beta) + \sa{\bar{X}}\sa{\bar{x}}\exists\bar{y}\bigvee_{i = 1}^m\varphi_i'(\bar{X},\bar{x},\bar{y})$.
