...
\martin{no revisar, es un copy-paste de arriba}	
...
\subsection{Proof of Theorem \ref{eqso-sigma-zero-in-fp}}
...
%
%We have that for each function $g \in \#\Sigma_1$ and structures $\A_1,\A_2\in\str$, if $\A_1$ is an induced substructure of $\A_2$, then $g(\A_1) \leq g(\A_2)$ \cite{SalujaST95}. Therefore, $f(\B) \geq f(\A) = 1$. However, there is no assignment $s\in B$ to $x$ such that $\B\models\:\varphi(s)$, so $f(\B) = 0$, which leads to a contradiction.
...
%	\begin{multline*}
%	\alpha_{\min}(\u_1,\dots,\u_n) = \exists\y\Big[ \alpha(\u_1,\dots,\u_n,\y)\wedge \\ \forall\v_1\cdots\forall\v_n\forall\w\Big(\big(\alpha(\v_1,\dots,\v_n,\w)\wedge\bigvee_{i=1}^n(\u_i\neq\v_i)\big)\to (\u_1,\dots,\u_n)<(\v_1,\dots,\v_n)\Big)\Big],
%	\end{multline*}
%	where $\alpha(\x) = \theta(\x) \wedge \beta(\x)$. Note that $\alpha_{\min}$ is satisfied only by the lexicographically smallest assignment $(\d_1,\dots,\d_n)$ to $(\x_1,\dots,\x_n)$ such that $\A\models\theta(\d_1,\dots,\d_n,\l)$ and $\A\models\beta(\d_1,\dots,\d_n,\l)$ for some $\l \in A^p$. Our new formula is
%	\begin{multline}
...
%
%Let $f = f_{\varphi(\X,\z)}$ such that $\varphi(\X,\z)$ is defined by the grammar $E_2$. First we notice that, as stated in \cite{SalujaST95}, for every $\L$-formula $\varphi$, $f_{\varphi}\in\shp$.\\
%
...
%
%Now we notice that $\zeta = \exists\X\exists\x\exists\y\,\theta(\X,\x,\y)$ is an existential second-order FO-formula, for which $\A\models\zeta$ can be decided in polynomial time \cite{Gradel91}.
...
\vspace{1em}
We will now prove that $\shdhsat$ is hard for $\eqso$ over a signature $\R$ under parsimonious reductions. For each $\eqso(\ehorn)$ formula $\alpha$ over $\R$, we will define a procedure that computes a function $g_{\alpha}$. This function receives a finite $\R$-structure $\A$ and outputs an instance of $\shdhsat$ such that $\sem{\alpha}(\A) = \shdhsat(g_{\alpha}(\A))$. We suppose that $\alpha$ is in sum normal form and:
$$
\alpha = c + \sum_{i = 1}^{\text{\#clauses}} \sa{\bar{X}}\sa{\bar{x}}\hat{\varphi}_i(\bar{X},\bar{x}),
$$
where each $\hat{\varphi}_i$ is a $\ehorn$ formula.

Consider a finite $\R$-structure $\A$ with domain $A$. To simplify the proof, we extend our grammar to allow first-order constants. Consider each tuple $\bar{a}\in A^{\length(\bar{x})}$ as a tuple of first-order constants. The following formula defines the same function as $\alpha$:
$$
c + \sum_{i = 1}^{\#clauses} \sum_{\bar{a}\in A^{\length(\bar{x})}} \sa{\bar{X}}\hat{\varphi}_i(\bar{X},\bar{e}).
$$
We redefine this formula as $c + \sum_{i = 1}^{m} \sa{\bar{X}}\hat{\psi}_i(\bar{X})$, where each $\hat{\psi}_i$ is a formula in $\ehorn$ with the extended grammar.

%\vspace{1em}
%Let $\alpha$ be equal to some constant $c$. We define $g_{\alpha}$ as a function that, for each $\A\in\ostr[\R]$, outputs the following formula with exactly $c$ satisfying assignments:
%$$
%\bigvee_{i = 1}^{c}\neg p_1 \wedge \cdots \wedge \neg p_{i-1} \wedge p_i \wedge \neg p_{i+1} \wedge \cdots \wedge \neg p_c.
%$$ 

Let the formula be equal to $c + \sum_{i = 1}^{m}\sa{\bar{X}}\exists\bar{y}\bigwedge_{j = 1}^{n}\forall\bar{z}\,\varphi^i_j(\bar{X},\bar{y},\bar{z})$ where each $\varphi_i$ is a Horn clause. The following formula defines the same function as $\alpha$:
$$
c + \sum_{i = 1}^{m}\sa{\bar{X}}\bigvee_{\bar{b}\in A^{\length(\bar{y})}}\bigwedge_{j = 1}^{n}\bigwedge_{\bar{c}\in A^{\bar{z}}}\varphi^i_j(\bar{X},\bar{b},\bar{c}).
$$
Note that each $\fo$ formula in each $\varphi^i_j$ has no free variables. Therefore, we can evaluate each of these in polynomial time and replace them by $\perp$ and $\top$ where it corresponds. Each $\varphi^i_j$ will be of the form $\perp \vee\, \chi^i_j(\bar{X})$ or $\top \vee \chi^i_j(\bar{X})$ where $\chi^i_j$ is a disjunction of $\neg X_{\ell}$'s and zero or one $X_{\ell}$. The formulas of the form $\top \vee \chi^i_j(\bar{X})$ can be removed entirely, and the formulas of the form $\perp \vee\, \chi^i_j(\bar{X})$ can be replaced by $\chi^i_j(\bar{X})$. We obtain the formula
$$
...
We make the following observation. If $\alpha$ is equal to $\sa{\bar{X}}\bigvee_{j = 1}^{\#d}\bigwedge_{k = 1}^{\#c}\psi_{j,k}(\bar{X})$, then the {\bf propositional formula} defined by $\bigvee_{j = 1}^{\#d}\bigwedge_{k = 1}^{\#c}\psi_{j,k}(\bar{X})$ over the propositional alphabet $\{X(\bar{e}) \mid X \in \bar{X} \text{ and } \bar{e}\in A^{\arity(X)} \}$ has exactly $\sem{\alpha}(\A)$ assignments and is a disjunction of Horn formulas. 
