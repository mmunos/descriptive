...
For condition (1) in \ref{cap-def}, let $\alpha \in \eqso(\fo)$ be defined over a relational signature $\R$. Let $\ell$ be the number of algebraic operators in $\alpha$. We will prove by induction over $\ell$ that there is a $\np$ machine $M$ such that $\alpha(\enc(\A))$ is equal to the number of accepting paths of $M$ on input $\enc(\A)$ for every $\A\in\ostr[\R]$. Let $\ell = 0$. Then $\alpha$ is either a formula in $\fo$ or a constant, and both functions can be easily computed in polynomial time. Suppose the hypothesis holds for $\ell$ from 1 through $n-1$. Let $\alpha$ have $n$ algebraic operators and let $\A$ be an ordered structure over $\R$ with domain $A$. If $\alpha = (\beta \add \gamma)$, then there exists a $\np$ machine for both $\beta$ and $\gamma$. Consider a machine that chooses between 0 or 1, if it is 0 (1), it calculates the function for $\beta$ ($\gamma$). The number of accepting paths in this $\np$ machine is equal to the function for $(\beta \add \gamma)$. If $\alpha = (\beta \mult \gamma)$, consider a machine that evaluates $\beta$, and in each accepting path, it continues evaluating $\gamma$. The number of accepting paths in this $\np$ machine is equal to the function for $(\beta \mult \gamma)$. If $\alpha = \sa{x}\beta$, consider a machine that chooses some $a\in A$ and then evaluates $\beta$ over $a$. If $\alpha = \pa{x}\beta$, consider a machine that evaluates $\beta$ consecutively over every element in $A$. If $\alpha = \sa{X}\beta$, consider a machine that guesses $P \subseteq A^{arity(X)}$ and then evaluates $\beta$ over $P$. This covers all possible cases for $\alpha$. Let $f$ be the function that this machine computes. We have that $f$ is a $\shp$ function over $\R$ and that for every $\A\in\ostr[\R]$: $f(\enc(\A)) = \alpha(\enc(\A))$.

For condition (2) in \ref{cap-def}, note that Saluja et al. \cite{DBLP:journals/jcss/SalujaST95} proved that $\shp = \#\fo$. We also have that $\sqso(\fo)$ captures $\#\fo$ over ordered structures so for each $f\in \shp$ let $\alpha \in \sqso$ be its corresponding formula. Since this formula is also in $\eqso$, the condition holds.
