...
Now let $\bar{u}$ be the tuple of every first-order variable mentioned in $\varphi_i^{+}$ and let $\bar{v}$ be such that $(\bar{x},\bar{y}) = (\bar{u},\bar{v})$. We define a formula $\mu_i$ that is satisfied only by the lexicographically smallest $\bar{u}$ that satisfies $\varphi_i^{+}$ for some $\bar{v}$:
$$
\mu_i(\bar{u}) = \exists\bar{v}\,\varphi_i^{\fo}(\bar{u},\bar{v})\wedge\forall\bar{u}'\forall\bar{v}'(\varphi_i^{\fo}(\bar{u}',\bar{v}')\to\bar{u}\leq\bar{u}').
$$
Lastly, we define $\kappa(\varphi_i)$:
$$
\kappa(\varphi_i) := 
$$
