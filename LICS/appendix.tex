...
We denote $(\varphi \mapsto \alpha)$ for $(\neg\varphi + (\varphi\cdot\alpha))$. Note that $\sem{(\varphi \mapsto \alpha)}(\A,v,V)$ equals $\sem{\alpha}(\A,v,V)$ when $(\A,v,V)\models\varphi$ and 1 otherwise, for each $(\A,v,V) \in \ostr[\R]^*$.
