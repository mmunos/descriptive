...
$\totp$ is an important counting complexity class for which we are missing a logical characterization. In the same way, we are missing logical characterization of harder fundamental complexity class such as $\spanl$. We would also like to define a larger syntactic subclass of $\shp$ where each function admits an FPRAS; notice that $\cpm$ is an important problem admitting an FPRAS\cite{JSV04} that is not included in the classes defined in Section \ref{sec-clo}. Moreover, by following the approach proposed in
