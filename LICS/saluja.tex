...
Unfortunately, for some $\LL$ there exist formulas in $\eqso(\LL)$  that cannot be rewritten in $\LL$-PNF.
...
\end{itemize}
\end{proposition}
The relationship between both hierarchies is summarized in Figure \ref{fig-sfo-eqso}.
One can see there that the difference between our hierarchy and the one proposed in~\cite{SalujaST95} only differ in~$\Sigma_0$ and~$\Sigma_1$. 
Actually, we show next that this difference is very crucial for finding classes of functions with easy decision versions and good closure properties.
