...
%, which is 
denoted by $\arity(h)$. Then the set of $\fqso$ formulae over a relational signature $\R$ is defined by the following grammar:
\begin{multline}
\label{eq-fqso}
...
More precisely, the set of $\rqfo(\fo)$ formulae over a relational signature $\R$, where $\rqfo$ stands for recursive $\qfo$, is defined as an extension of $\qfo(\fo)$ that includes the formula 
