...
%, which is 
denoted by $\arity(h)$. Then the set of $\fqso$ formulas over a relational signature $\R$ is defined by the following grammar:
\begin{multline}
\label{eq-fqso}
...
the classical notion of function symbol in $\fo$~\cite{enderton2001mathematical}. 
...
Given a relation signature $\R$, the set of transitive $\qfo$ formulas ($\tqfo$-formulas) is defined as an extension of $\qfo$ with the formula
%\begin{multline}
%	\label{eq-def-tqso}
...
\end{multline*}
where $\varphi_{\text{\rm first-lex}}$ and $\varphi_{\text{succ-lex}}$ are $\fo$-formulas defining the first and successor predicates over tuples in $A^k$, following the lexicographic order induced by~$<$.
...
As a example showing how fundamental this separation is, consider the issue of extending $\qfo(\fo)$ at the Boolean level in order to capture $\shl$. The natural alternative to do this is to use $\fo$ extending with a transitive closure operator, which is denoted by $\tc$. But then the problem is that for every language $L \in \nlog$, it holds that its characteristic function $\chi_L \in \qfo(\tc)$, where $\chi_L(w) = 1$ if $w \in L$, and $\chi_L(w) = 0$ otherwise. Thus, if we assume that 
