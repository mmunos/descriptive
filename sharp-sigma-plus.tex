...
$$
will be satisfied by exactly one predicate less than $\exists \vec{x} \varphi_1(\vec{x},\vec{P})$. If $\varphi'_i(\vec{x})$ is unsatisfiable, we eliminate one predicate on the next $\varphi_i$. If this sub-formula is also unsatisfiable, we move to the next one, and so on. Therefore:
\begin{multline*}
F_{-1}(\A) = \vert \{ \langle\vec{P}\rangle :\A \models 
...
\exists \vec{x} \left( \varphi_k(\vec{x},\vec{P}) \wedge \left( (\forall \vec{x'} (\neg \varphi'_1(\vec{x'}) \wedge \neg \varphi'_2(\vec{x'}) \wedge ... \wedge \neg \varphi'_{k-1}(\vec{x'})) ) \rightarrow \psi_k(\vec{x}) \right)\right)
 \} \vert
\end{multline}
eliminates exactly one predicate of $F(\A)$.
\end{enumerate}
\item The counting set is:
$$
