...
A function $f:\Truc \to \N$ is in $\Efour$ if there exists an {\sc FO}-extended quantifier-free $\L$-formula $\varphi(\x^1,\x^2,\Y,\y)$ such that $f = f_{\forall\x^1\exists \x^2\,\varphi(\x^1,\x^2,\Y,\y)}$ and any second order variables only appear in Horn clauses. We will now show that {\sc \#DNF} $\in\Ethree$.
