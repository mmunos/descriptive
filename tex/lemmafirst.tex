...
\end{lemma}
\begin{proof}
Let $\A \in \Truc$. Consider two cases: 
\icristian{
Acá yo explicaría cuales son los dos casos para entender el roadmap de la demostración.	
}
...
\icristian{Este argumento es uno de los principales. Si cambiamos el orden de la demostración (colocando la descomposicion DNF primero) esta demo se podría colocar en el Claim 5.1 y después reusar acá}
...
\icristian{
Aqui yo recalcaría que 	$\A$ no modela la segunda parte de $\varphi^\prime(\x,\P)$.
}
\item[(c)] By contradiction, let $\P^\prime \neq \P$ be such that $\A\models\exists\x\,\varphi(\x,\P^\prime)$ and $\A\not\models\exists\x\,\varphi^\prime(\x,\P^\prime)$. We consider two cases: first, suppose that $\P^\prime$ is missing a tuple of $\P$. Let $i\in\{1,\ldots,n\}$ such that in $\d_i$ is not in $P^\prime_{\lambda(i)}$. Then $\A\models \neg P^\prime_{\lambda(i)}(\d_i)$, and so,
$$\A\models \bigg(\bigvee_{i = 1}^{n}\neg X_{\lambda(i)}(\d_i) \bigg), $$ 
from which we conclude that $\A\models\exists\x\,\varphi^\prime(\x,\P^\prime)$. Second, suppose there is some predicate $\P^\prime_i$ in $\P^\prime$ which has a tuple that $P_i$ does not have. If this is the case, then $$\A\models\bigvee_{i=1}^r \exists \v\Big( P'_i(\v) \wedge \bigwedge_{j\in[1,n]:\: \lambda(j) = i} \v \neq \u_j\Big),$$ so $\A\models\exists\x\,\varphi^\prime(\x,\P^\prime)$. On both cases, we have a contradiction.
\icristian{
Esta demostración es mas clara si es directa, diciendo que para todo $\P^\prime \neq \P$ se cumple que $\A\models\exists\x\,\varphi(\x,\P^\prime)$ y $\A\models\exists\x\,\varphi^\prime(\x,\P^\prime)$.
}
\end{enumerate}
