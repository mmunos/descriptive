...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item There is only second-order free variables in $\psi$. Let $\psi = \exists \x\,\varphi(\x,\X)$, where $\x = (x_1,\ldots,x_c)$ and $\X = (X_1,\ldots,X_r)$. Given a structure $\A = \langle A, \S^{\A}, \leq^{\A} \rangle\in\Truc$, we generate a DNF-formula $\Phi = h(\A)$ as follows:
