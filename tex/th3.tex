...
He have that for each function $g \in \#\Sigma_1$ and structures $\A_1,\A_2\in\Truc$, if $\A_1$ is an induced substructure of $\A_2$, then $f(\A_1) \leq f(\A_2)$ \cite{DBLP:journals/jcss/SalujaST95}. Therefore, $g(\B) \geq g(\A) = 1$. However, there is no assignment $s\in B$ to $x$ such that $\B\models\:\varphi(s)$, so $f(\B) = 0$, which leads to a contradiction.
\end{proof}